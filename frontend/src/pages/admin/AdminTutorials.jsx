...
const AdminTutorials = () => {
  ...
  const fetchTutorials = async () => {
    try {
      const res = await api.get('/tutorials/admin');
      if (res.data.success) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const payload = { title, slug, description, level, published };
    try {
      if (editingId) {
        await api.put(`/tutorials/admin/${editingId}`, payload);
      } else {
        await api.post('/tutorials/admin', payload);
      }
      setIsModalOpen(false);
      fetchTutorials();
    } catch (error) {
      alert(error.response?.data?.error || 'Failed to save tutorial');
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this tutorial?')) {
      try {
        await api.delete(`/tutorials/admin/${id}`);
        fetchTutorials();
      } catch (error) {
  ...
  if (loading) return <div className="text-slate-400">Loading tutorials...</div>;

  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold text-white">Manage Tutorials</h1>
        <button 
          onClick={() => openModal()}
          className="bg-gradient-neon text-white px-4 py-2 rounded-lg font-medium flex items-center gap-2 hover:opacity-90"
        >
          <Plus className="w-5 h-5" /> New Tutorial
        </button>
      </div>
        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-dark-border">
            {tutorials.map((tutorial) => (
              <tr key={tutorial._id} className="hover:bg-dark-700/50 transition-colors">
                <td className="px-6 py-4 text-white font-medium">{tutorial.title}</td>
                <td className="px-6 py-4 text-slate-400 capitalize">{tutorial.level}</td>
                <td className="px-6 py-4">
                  <span className={`px-3 py-1 rounded-full text-xs font-medium ${tutorial.published ? 'bg-purple-glow/10 text-purple-glow border border-purple-glow/20' : 'bg-slate-800 text-slate-400 border border-slate-700'}`}>
                    {tutorial.published ? 'Published' : 'Draft'}
                  </span>
                </td>
                <td className="px-6 py-4 text-right space-x-3">
                  <button onClick={() => openModal(tutorial)} className="text-slate-400 hover:text-cyan-glow">
                    <Edit2 className="w-5 h-5 inline" />
                  </button>
                  <button onClick={() => handleDelete(tutorial._id)} className="text-slate-400 hover:text-red-400">
            ...
            {tutorials.length === 0 && (
              <tr><td colSpan="4" className="px-6 py-8 text-center text-slate-500">No tutorials found.</td></tr>
            )}
          </tbody>
        </table>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-dark-900/80 backdrop-blur-sm">
          <div className="bg-dark-800 border border-dark-border rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-dark-border flex justify-between items-center sticky top-0 bg-dark-800 z-10">
              <h2 className="text-xl font-bold text-white">{editingId ? 'Edit Tutorial' : 'New Tutorial'}</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <select value={level} onChange={(e) => setLevel(e.target.value)} className="w-full bg-dark-900 border border-dark-border rounded-lg px-4 py-2 text-white focus:outline-none focus:border-cyan-glow">
              ...
              <div className="flex items-center gap-3 py-2">
                ...
                <label htmlFor="published" className="text-slate-300 font-medium">Publish</label>
              </div>
              <div className="pt-4 flex justify-end gap-3">
                ...
                <button type="submit" className="bg-gradient-neon text-white px-6 py-2 rounded-lg font-medium hover:opacity-90">Save Tutorial</button>
              </div>
            </form>
          </div>
        </div>
